...
void push_element(stack **head, float data);   //описание используемых фунцкий
...
setvbuf(stderr, NULL, _IONBF, 0);
	...
	FILE *input, *output;
	while (n == 'y') {
		printf("Enter name of input file:"); // запрос об наименовании файла при запуске
		scanf(" %s", in);
		...
		while (feof(input) == 0) {
			...
			if (str[i] == ' ') {
				if (t == '0') {
					num = strtof(str, NULL); // деление строки на отдельные элементы
					push_element(&head_stack, num);
					i = 0;
				}
			} else if (str[i] == '*' || str[i] == '+' || str[i] == '-' //поиск и выполнение операции с найденными элементами
					|| str[i] == '/' || str[i] == '^') {
				...
				t = '1';
			} else if (str[i] == 'n') { //поиск элемента, прерывающего выполнение функции
...
//Вычислительный блок калькулятора
float numb(char sign, float x, float y) {
	float res, f, S;
	switch (sign) {
	case '+':
...
void push_element(stack **head, float data) { //перемещает элемент на вершину стека

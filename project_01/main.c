...
// Блок 1:подключаем библиотеки
#include <stdio.h>
#include <stdlib.h>
//
// Блок 2:организовываем список для входных данных
struct list1 {
	char sign, choose;
	int size;
	float *x, *y;
...
//
// Блок 3:организовываем список для выходных данных
struct list2 {
	float *res;
...
int main() {
	...
	while (n == 'y') {
		...
		if (feof(input) == 0) {
			...
			fclose(input);
			output = fopen(out, "w");
			while (current != NULL) 			//запись ответа в файл "output"
			{
				if (current->choose == 'v') {
					fprintf(output, "(");
